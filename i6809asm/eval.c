...
int get_term()
{
        ...
        if(*Optr=='%'){ /* binary constant */
        ...
        else if( any(*Optr,"0123456789")) { /* decimal constant or 0xYYYY hexa value*/

			//	hexa value starting with '0x'
			if(*Optr == '0' && *(Optr+1) != EOS && *(Optr+1) == 'x' ) {
			...
			//	optional quote final: example: lda #'A   or lda #'A'
			if( *Optr == '\'' ) {
				Optr++;
			}
        }
